...
#Przygotowanie tabeli z danymi potrzebnymi do przeprowadzenia analizy.</br>
...
- Eta4_1 - Wiek respondentów zebranych według przedziałów wiekowych: 
</br>0-17 (1) 
</br>18-34 (2) 
</br>35-65 (3) >65 (4)
...
</br>Sprawdzamy czy w danych nie występują braki:
```{r echo=FALSE}
md.pattern(wydatkiMieso, rotate.names = T)
```
</br>W danych nie występują braki.
...
*Występują duże różnice pomiędzy wartością maksymalną, minimalną oraz średnią w poszczególnych kategoriach, a także kolumnie zbiorowej. Różnice te wskazują na występowanie znacznych wartości odstających, które należy usunąć przed przystąpieniem do dalszej analizy* </br>
...
#Inforamcje o próbie. Wstępne statystyki.
</br>
...
<br/> Z powyższego histogramu i tabeli wynika, że w badaniu nie wzięły udziału rodziny oznaczone według typu:
  <br/>- 2 - Osoba samotna w wieku 35-64 lat
  <br/>- 5 - Para bez dzieci z osobą kontaktową w wieku 35-64 lat
  <br/>- 9 - Para z 3 i więcej dziećmi
  <br/>- 11 - inne typy
...
Z powyższego histogramu wynika, że w badaniu nie wzięły udziały gospodarstwa domowe, które liczą więcej niż 6 członków. Jest to informacja wymagajca dalszego zweryfikowania, ponieważ we wczeniejsze analize wykazano, że w badaniu nie występuje typ rodziny para z trójką trójk i wicej dziećmi. Tworzymy tabelę kontyngencji dla typów rodzin oraz ilości osób w domostwie.
...
Wydatki na wedniliny w rodznie z 1 dzieckiem.
...
```{r echo=FALSE}
ostatnie_obs <- quantile(dane_model_pelne$Suma_wydatków, c(0.97, 0.98, 0.99))
ostatnie_obs
```
...
(sr_wydatki_wedliny <- dane_model_grupy_bez25 %>%
  group_by(quantile) %>%
...
```{r echo=FALSE}
#REGRESJA 
#X - dochod na glowe, tu dochod rozpozadzalny na glowe
#Y - wydatki na wedliny na glowe
#C - poziom dochodu przy ktorym pojawia sie wydatek - 3315.39

Srednie_Wydatki <- dane_model_czyste %>%
  select(Suma_wydatków) %>%
  mutate(quantile = ntile (Suma_wydatków, 25)) 
  
Srednie_Wydatki<- Srednie_Wydatki%>%
  group_by(quantile) %>%
  summarise(sr_wydatki = mean(Suma_wydatków))

Srednie_Wydatki_model <- as.data.frame(cbind(Srednie_Wydatki$sr_wydatki,sr_wydatki_wedliny$srednie_w_gr_dochodowej))
colnames(Srednie_Wydatki) <- c("sr_wydatki","sr_wydatki_wedliny")
```

```{r echo=FALSE}

m <- nls(Wydatki_na_os_wedliny ~ (A * Wydatki_na_os * (Wydatki_na_os - C) / (B + Wydatki_na_os)), data = dane_model_czyste, start = list(A = 19, B = 200,C = 3315))
