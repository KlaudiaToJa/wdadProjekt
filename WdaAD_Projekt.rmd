...
Czy to, że rodzina przekracza średnią miesięczną wysokość wydatków na mięso, jest zależne od typu rodziny, miesiąca lub ilości osób w domostwie (MOŻNA DODAĆ JESZCZE INNE OPCJE)?
...
doLogistycznej$doMiesaRazem <- ifelse(doLogistycznej$miesoRazem>=srednia, 1, 0) # jeśli wydali więcej niż wynosi średnia, zastępuję 1
```


```{r}
wynikLogist1 <- glm(formula = doMiesaRazem ~ TIPFAM + Kwartal + NC, data = doLogistycznej, family = binomial)
summary(wynikLogist1)
```

Zgodnie z wynikami powyższej regresji, fakt, czy rodzina przekracza średnią miesięczną wysokość wydatków jest istotnie zależny od typu rodziny oraz ilości osób w rodzinie.
